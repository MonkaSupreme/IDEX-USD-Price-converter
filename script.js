function updateDollarPrice(){
  $.post( 'https://api.idex.market/returnTicker').done(function(data){
    $.get("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(cmcData, status){
       ...
       $("#volumeDUBI_ETH").html(parseFloat(data.ETH_DUBI.baseVolume).toFixed(3))
       ...
       $("#volumeDUBI_Dollar").html("$" + (parseFloat(data.ETH_DUBI.baseVolume) * ethPrice).toFixed(2))
       ...
       $("#volumePRPS_Dollar").html("$" + (parseFloat(data.ETH_PRPS.baseVolume) * ethPrice).toFixed(2))
     })
  })
}
